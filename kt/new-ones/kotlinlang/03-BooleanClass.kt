...
class BooleanClass {
    ...
    fun solve(taskNo: Int): Boolean {
        val path = "src/pack03boolean/tests/Boolean$taskNo/"
        var fileName: String
        for (index in 1..100) {
            fileName = path + "%03d".format(index)
            if (!datScanner.changeReader("$fileName.dat")) {
                println("Boolean$taskNo couldn't open datScanner source")
                return false
            }
            if (!ansScanner.changeReader("$fileName.ans")) {
                println("Boolean$taskNo couldn't open ansScanner source")
                return false
            }
            val ok = when (taskNo) {
                1 -> this.boolean01()
            ...
            scanner.removeFiles()
            if (!ok) {
                println("Boolean$taskNo Test#$index has not been tested")
                return false
            }
            if (!datScanner.eof()) {
                println("Boolean$taskNo Test#$index input is not enough")
                return false
            }
            if (!ansScanner.eof()) {
                println("Boolean$taskNo Test#$index output is not enough")
                return false
            }
        }
        println("Boolean$taskNo Tested!")
        return true
    }

    private fun boolean01(): Boolean {
        val a = this.datScanner.nextInt()
        val isPositive = a > 0
        return this.ansScanner.checkBoolean(isPositive)
    }

    private fun boolean02(): Boolean {
        val a = this.datScanner.nextInt()
        val isOdd = a % 2 != 0
        return this.ansScanner.checkBoolean(isOdd)
    }

    private fun boolean03(): Boolean {
        val a = this.datScanner.nextInt()
        val isEven = a % 2 == 0
        return this.ansScanner.checkBoolean(isEven)
    }

    private fun boolean04(): Boolean {
        val a = this.datScanner.nextInt()
