...
class CaseClass {
    ...
    fun solve(taskNo: Int): Boolean {
        val path = "src/pack05case/tests/Case$taskNo/"
        var fileName: String
        for (index in 1..100) {
            fileName = path + "%03d".format(index)
            if (!datScanner.changeReader("$fileName.dat")) {
                println("Case$taskNo couldn't open datScanner source")
                return false
            }
            if (!ansScanner.changeReader("$fileName.ans")) {
                println("Case$taskNo couldn't open ansScanner source")
                return false
            }
            val ok = when (taskNo) {
                1 -> this.case01()
            ...
            scanner.removeFiles()
            if (!ok) {
                println("Case$taskNo Test#$index has not been tested")
                return false
            }
            if (!datScanner.eof()) {
                println("Case$taskNo Test#$index input is not enough")
                return false
            }
            if (!ansScanner.eof()) {
                println("Case$taskNo Test#$index output is not enough")
                return false
            }
        }
        println("Case$taskNo Tested!")
        return true
    }

    private fun case01(): Boolean {
