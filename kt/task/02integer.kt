    ...
    fun solve(pathPrefix: String) {
        for (taskNo in 1..30) {
            ...
            for (testNo in 1..100) {
                ...
                fileName = "$pathPrefix/02integer/Integer$taskNoStr/$testNoStr/$testNoStr"
                if (!datScanner.changeReader("$fileName.dat")) {
                    println("Integer$taskNoStr couldn't open datScanner source")
                    return
                }
                if (!ansScanner.changeReader("$fileName.ans")) {
                    println("Integer$taskNoStr couldn't open ansScanner source")
                    return
                }
                val ok = when (taskNo) {
                    1 -> this.integer1()
                ...
                scanner.removeFiles()
                if (!ok) {
                    println("Integer$taskNoStr Test#$testNoStr has not been tested")
                    return
                }
                if (!datScanner.eof()) {
                    println("Integer$taskNoStr Test#$testNoStr input is not enough")
                    return
                }
                if (!ansScanner.eof()) {
                    println("Integer$taskNoStr Test#$testNoStr output is not enough")
                    return
                }
            }
            println("Integer$taskNoStr has been tested!")
        }
    }

    private fun integer1(): Boolean {
        val l = this.datScanner.nextInt()
        val meters = l / 100
        return this.ansScanner.checkInt(meters)
    }

    private fun integer2(): Boolean {
        val m = this.datScanner.nextInt()
        val kg = m / 1000
        return this.ansScanner.checkInt(kg)
    }

    private fun integer3(): Boolean {
        val b = this.datScanner.nextInt()
        val kBytes = b / 1024
        return this.ansScanner.checkInt(kBytes)
    }

    private fun integer4(): Boolean {
