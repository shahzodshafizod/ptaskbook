    ...
    fun solve(pathPrefix: String) {
        for (taskNo in 1..30) {
            ...
            for (testNo in 1..100) {
                ...
                fileName = "$pathPrefix/04if/If$taskNoStr/$testNoStr/$testNoStr"
                if (!datScanner.changeReader("$fileName.dat")) {
                    println("If$taskNoStr couldn't open datScanner source")
                    return
                }
                if (!ansScanner.changeReader("$fileName.ans")) {
                    println("If$taskNoStr couldn't open ansScanner source")
                    return
                }
                val ok = when (taskNo) {
                    1 -> this.if1()
                ...
                scanner.removeFiles()
                if (!ok) {
                    println("If$taskNoStr Test#$testNoStr has not been tested")
                    return
                }
                if (!datScanner.eof()) {
                    println("If$taskNoStr Test#$testNoStr input is not enough")
                    return
                }
                if (!ansScanner.eof()) {
                    println("If$taskNoStr Test#$testNoStr output is not enough")
                    return
                }
            }
            println("If$taskNoStr has been tested!")
        }
    }

    private fun if1(): Boolean {
