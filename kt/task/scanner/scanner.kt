...
class Scanner {
    ...
    fun checkFloat(precision: Int, vararg args: Float): Boolean {
        ...
        for (arg in args) {
            ...
            argValue = arg.round(precision)
            if (value != argValue) {
                println("expected: $value")
                println("have got: $argValue")
                println("have got: $arg")
                return false
            }
        }
    ...
    fun checkBinaryFile(endian: ByteOrder, vararg fileNames: String): Boolean {
        ...
        for (fileName in fileNames) {
            ...
            val correctChs = getCheckSum(correctFileName)
            val checkChs2 = getCheckSum(fileName)
